 ...
 * Stacking. Building multiple models (typically of differing types) and supervisor model that learns how to best combine the predictions of the primary models. There are multiple layers in Stacking, the lower layers send their precition results to the above layer as features, <b>multiple model predictions are not highly correlated</b>. The top layer can also be Average/Majority Vote/Weighted Average
  ...
  * Blending: Similar to Stacking, but while Stacking uses hold-out data (only training and testing), Blending will split the training data into training & validation data, in each stage, the models are trained on the training data, tested on validation data and testing data, the prediction results for validation data will be put in the next stage training, the prediction results of testing data will be put into the testing data for the next stage.
